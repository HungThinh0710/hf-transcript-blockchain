...
exports.enrollAdmin = async (req, res) => {
    ...
    try {
        ...
        res.send(new ReturnResult(null, null, Constants.messages.ENROLL_ADMIN_SUCCESS, null));
    } catch (err) {
        console.log(err)
...
exports.registerUser = async (req, res) => {
    ...
    try {
        ...
        return res.send(new ReturnResult(result.identity, null, Constants.messages.REGISTER_USER_SUCCESS, null));
    } catch (err) {
        console.log(err)
        res.status(400).send(new ReturnResult(null, null, null, err.message, true));
        // process.exit(1);
