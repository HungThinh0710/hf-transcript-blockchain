...
jq '.channel_group.groups.Application.groups."it-vku-udn-vn".values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "192.168.93.168","port": 7003}]},"version": "0"}}' vku_current_config.json > vku_modified_anchor_config.json
...
configtxlator proto_encode --input vku_modified_anchor_config.json \
...
configtxlator compute_update --channel_id vku \
  ...
  --output vku_anchor_update.pb

# 7. Decode the delta of the config to json format
configtxlator proto_decode --input vku_anchor_update.pb \
  --type common.ConfigUpdate | jq . > vku_anchor_update.json

# 8. Now wrap of the delta config to fabric envelop block
echo '{"payload":{"header":{"channel_header":{"channel_id":"vku", "type":2}},"data":{"config_update":'$(cat vku_anchor_update.json)'}}}' | jq . > vku_anchor_update_envelope.json

# 9. Encode the json format into protobuf format
configtxlator proto_encode --input vku_anchor_update_envelope.json \
  --type common.Envelope --output vku_anchor_update_envelope.pb

# 10. Need to sign anchor update envelop by org admin
peer channel update -o $ORDERER_ADDRESS --tls --cafile $ORDERER_TLS_CA \
  -f vku_anchor_update_envelope.pb -c vku
