...
export ORDERER_ADDRESS=172.30.127.73:7010
export ORDERER_TLS_CA=/vars/keyfiles/ordererOrganizations/vku.udn.vn/orderers/orderer1.vku.udn.vn/tls/ca.crt

peer lifecycle chaincode queryinstalled -O json | jq -r '.installed_chaincodes | .[] | select(.package_id|startswith("transcript_1.1:"))' > ccstatus.json
...
export CORE_PEER_ADDRESS=172.30.127.73:7005

# approved=$(peer lifecycle chaincode checkcommitreadiness --channelID vku \
#   --name transcript --version 1.1 --init-required --sequence $SEQUENCE --tls \
